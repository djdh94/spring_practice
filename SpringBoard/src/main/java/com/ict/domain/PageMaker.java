...
public class PageMaker {
	...
	public void calcData() {
		// 한 페이지에 하단에 깔고싶은 글 개수
		this.displayPageNum=10;
		
		// 현재 페이지(cri.getPageNum())을 근거로 페이지 그룹중 끝나는 페이지 구함.
		...
		// next는 여태까지 출력한 페이지에 속한 글 개수보다 DB내 전체 글이 더 많은 경우에 활성화
		next = endPage * cri.getNumber() >= totalBoard ? false:true;
		
	}// calcData()
	public void setTotalBoard(int totalBoard) {
		this.totalBoard=totalBoard;
		calcData();// prev,next,endPage,startPge를 다 구함
	}
